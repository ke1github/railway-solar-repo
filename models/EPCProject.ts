...
EPCProjectSchema.methods.calculateHealthScore = function() {
  ...
  // Deduct for delays
  const now = new Date();
  if (this.resources.timeline.plannedEndDate < now && this.overallStatus !== 'completed') {
    score -= 20;
  }
  
  // Deduct for budget overruns
  const budgetUtilization = this.resources.budget.spent / this.resources.budget.total;
  if (budgetUtilization > 1.1) score -= 15;
  
  // Deduct for open high-risk items
  const highRisks = this.risks.filter((r: any) => r.probability === 'high' && r.status === 'open');
  score -= highRisks.length * 10;
  
  // Deduct for failed inspections
  const failedInspections = this.qualityControl.inspections.filter((i: any) => i.status === 'failed');
...
export default mongoose.models.EPCProject || mongoose.model<IEPCProject>('EPCProject', EPCProjectSchema);
