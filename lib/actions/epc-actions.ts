...
import EPCProject, { IEPCProject } from '@/models/EPCProject';
...
export async function createEPCProject(formData: FormData) {
  try {
    ...
    const projectData: EPCProjectFormData = {
      ...
      projectType: formData.get('projectType') as 'solar_installation' | 'maintenance' | 'upgrade' | 'expansion',
    ...
    // Generate project ID
    const projectId = `EPC-${Date.now()}-${Math.random().toString(36).substr(2, 4).toUpperCase()}`;
    ...
    const newProject = new EPCProject({
      ...
      phases: {
        engineering: {
          status: 'not_started',
          assignedTeam: projectData.engineeringTeam ? [projectData.engineeringTeam] : [],
          documents: [],
          progress: 0,
        },
        procurement: {
      ...
      risks: [
        {
          ...
          probability: 'medium',
          impact: 'medium',
          mitigation: 'Monitor weather forecasts and adjust schedule accordingly',
          status: 'open',
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Failed to create EPC project' 
    };
  }
}
...
export async function updateEPCProject(projectId: string, formData: FormData) {
  try {
    ...
    const updateData = {
      ...
      'resources.budget.total': parseFloat(formData.get('budgetTotal') as string),
      'resources.timeline.plannedStartDate': new Date(formData.get('plannedStartDate') as string),
      'resources.timeline.plannedEndDate': new Date(formData.get('plannedEndDate') as string),
      'phases.engineering.assignedTeam': formData.get('engineeringTeam') ? [formData.get('engineeringTeam')] : [],
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Failed to update EPC project' 
    };
  }
}

export async function updateProjectPhase(projectId: string, phase: 'engineering' | 'procurement' | 'construction', phaseData: FormData) {
  try {
    await connectToDatabase();
    ...
    if (status === 'in_progress' && phaseData.get('startDate')) {
      updateObj[`phases.${phase}.startDate`] = new Date(phaseData.get('startDate') as string);
    }

    if (status === 'completed' && phaseData.get('completedDate')) {
      updateObj[`phases.${phase}.completedDate`] = new Date(phaseData.get('completedDate') as string);
    }
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Failed to update project phase' 
    };
  }
}

export async function getEPCProjects(page: number = 1, limit: number = 20, status?: string, priority?: string) {
  try {
    await connectToDatabase();
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Failed to fetch EPC projects',
      projects: [],
...
export async function getEPCProjectById(projectId: string) {
  try {
    ...
    const overallProgress = project.phases ? Math.round(
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Failed to fetch EPC project' 
    };
  }
}
...
export async function getEPCDashboardStats() {
  try {
    ...
    const recentProjects = await EPCProject.find()
      .sort({ createdAt: -1 })
      .limit(5)
      .select('projectId projectName overallStatus priority healthScore createdAt')
      .lean();
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Failed to fetch EPC dashboard stats',
      stats: {
        overview: {
          totalProjects: 0,
          activeProjects: 0,
...
export async function deleteEPCProject(projectId: string) {
  try {
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Failed to delete EPC project' 
    };
  }
}
