...
export async function createSite(formData: FormData) {
  try {
    ...
    const siteData: Partial<SiteFormData> = {
      ...
      status: formData.get('status') as 'planning' | 'survey' | 'design' | 'construction' | 'operational' | 'maintenance',
    };
    ...
    const newSite = new RailwaySite({
      ...
      monthlyEnergyTarget: siteData.feasibleCapacity ? siteData.feasibleCapacity * 4 * 30 : 0,
      carbonOffsetKg: siteData.feasibleCapacity ? siteData.feasibleCapacity * 0.8 * 365 : 0,
...
export async function getSites(page: number = 1, limit: number = 20, search?: string, cluster?: string) {
  try {
    await connectToDatabase();
